  ...
  return (
    <main>
      ...
      <div className='output overflow-y-auto'>
        {loading ? (<Loading />) : (tags?.map((tag) => (
          <DockerTag key={tag.id} tag={tag} />
        )))}
      </div>
    </main>
  )
  ...
  return (
    <div className='w-full py-2'>
      ...
      {images?.map((image: any) => (
        <div key={image.digest} className='flex flex-row justify-between'>
          <span>{image.os}/{image.architecture}</span>
          <span>{prettyBytes(image.size, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      ))}
    </div>
  )
